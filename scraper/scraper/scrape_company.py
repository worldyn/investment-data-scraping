...
def parse_company_page(page_source, company):
    ...
    #with open("cpage.html", "w", encoding='utf-8') as file:
    #    file.write(str(soup))
    ...
    def add_members(ul):
        # Re-using parsing format for similar divs
        # Only used in loop below to get mgmt and board
        member_list = []
    ...
    if divs[0].find('ul'):
        ...
        '''
        company_web = divs[0].find('ul').find_all(
            'li', class_='flex',recursive=False
        )[4].find('a')['href']
        '''
...
def main():
    parser = argparse.ArgumentParser(description='Get additional data from specific company. You must set input and output json file names')
